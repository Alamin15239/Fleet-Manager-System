...
export async function POST(request: NextRequest) {
  try {
    ...
    const verificationToken = resendEmailService.generateEmailToken()
    await resendEmailService.storeEmailToken(user.id, verificationToken)
    
    try {
      await resendEmailService.sendVerificationEmail(email, verificationToken, name)
    } catch (emailError) {
